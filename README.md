...
Names reflect your Eclipse screenshot; adjust if any file name differs (e.g., forgetPassword.html).
...
Push to GitHub → Cloud Build (Buildpacks/Java)

Cloud Run service (region: asia-northeast1)

Networking: attach VPC Connector, set egress = Route all traffic

Allow NAT IP on the MySQL VM (firewall + MySQL user host)
...
GitHub へ push → Cloud Build が自動ビルド

Cloud Run サービスを作成（リージョン asia-northeast1）

VPC コネクタを接続し、すべてのトラフィックを VPC にルーティング

Cloud NAT の外部 IP を MySQL 側（FW とユーザーの host）に許可
